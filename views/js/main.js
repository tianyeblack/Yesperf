...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        // Only query the random pizzas one time to get all of them
        var containers = document.getElementsByClassName("randomPizzaContainer");
        // First compute all the width changes
        var newwidths = [];
        for (var i = 0; i < containers.length; i++) {
            var dx = determineDx(containers[i], size);
            newwidths[i] = (containers[i].offsetWidth + dx) + 'px';
        }
        // Then apply them in a batch
        for (var i = 0; i < containers.length; i++) {
            containers[i].style.width = newwidths[i];
        }
    }
